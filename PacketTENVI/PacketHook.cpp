...
void __fastcall COutPacket_Hook(OutPacket *p, void *edx, BYTE b, void *v) {
	packet_id_out++;
	PacketExtraInformation pxi = { packet_id_out, (ULONG_PTR)_ReturnAddress(), ENCODEHEADER, 0, sizeof(BYTE) };
	AddExtra(pxi);
...
WORD __fastcall DecodeHeader_Hook(InPacket *p, void *edx) {
	if (p->decoded == 0x04) {
		PacketExtraInformation pxi = { packet_id_in, (ULONG_PTR)_ReturnAddress(), DECODEHEADER, p->decoded - 4, sizeof(BYTE) };
		AddExtra(pxi);
	}
...
WCHAR** __fastcall DecodeWStr1_Hook(InPacket *p, void *edx, WCHAR **wc) {
	PacketExtraInformation pxi = { packet_id_in, (ULONG_PTR)_ReturnAddress(), DECODEWSTR_1, p->decoded - 4, sizeof(BYTE) + p->packet[p->decoded] * 2 };
	AddExtra(pxi);
	return _DecodeWStr1(p, wc);
}
WCHAR** __fastcall DecodeWStr2_Hook(InPacket *p, void *edx, WCHAR **wc) {
	PacketExtraInformation pxi = { packet_id_in, (ULONG_PTR)_ReturnAddress(), DECODEWSTR_2, p->decoded - 4, sizeof(WORD) + *(WORD *)&p->packet[p->decoded] * 2 };
	AddExtra(pxi);
