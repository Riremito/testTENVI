...
bool PacketSender(Alice &a, MessageHeader type) {
	...
	wpacket.erase(wpacket.begin(), wpacket.begin() + 2);
	...
	while (psh.Recv(data)) {
		...
		if (pem.header == SENDPACKET) {
			if (a.CheckBoxStatus(CHECK_SEND) && !SearchHeaders(vIgnoreSendHeaders, *(WORD *)&pem.Binary.packet[0])) {
				...
				// @header data
				//wpacket.erase(wpacket.begin(), wpacket.begin() + 3); // 5 -> 3
					wpacket = L"@" + wpacket;
...
std::wstring GetFormat(PacketData &pd, PacketFormat &fmt) {
	...
	try {
		...
		case ENCODEHEADER:
		{
			...
			wText += L"@";
			if (fmt.size == 1) {
				wText += BYTEtoString(pd.packet[fmt.pos]);
			}
			else
			{
		...
		case DECODEWSTR_1:
		{
			wText += L"WStr1(" + std::to_wstring((fmt.size - sizeof(BYTE)) / 2) + L")\r\n\t";
			std::wstring utf16 = std::wstring((WCHAR *)&pd.packet[fmt.pos + sizeof(BYTE)], pd.packet[fmt.pos]);
			wText += L"L\"" + utf16 + L"\"";
			break;
		}
		case DECODEWSTR_2:
		{
			wText += L"WStr2(" + std::to_wstring((fmt.size - sizeof(WORD)) / 2) + L")\r\n\t";
			std::wstring utf16 = std::wstring((WCHAR *)&pd.packet[fmt.pos + sizeof(WORD)], *(WORD *)&pd.packet[fmt.pos]);
			wText += L"L\"" + utf16 + L"\"";
			break;
		}
		case DECODEBUFFER:
...
#ifdef _WIN64
	Alice a(L"PacketEditorClass64", L"Test Rire PE x64", 1024, 768, hInstance);
#else
	Alice a(L"PacketEditorClass", L"Test Rire PE x86", 1024, 768, hInstance);
#endif
